...
class MultiHeadMultiScaleAttention(nn.Module):
        ...
        for kernel_sizes in head_kernel_sizes:
            ...
            for k in kernel_sizes:
                if k % 2 == 0:
                    # 偶数卷积核需要特别处理以保持输出尺寸
                    padding = (k // 2, k // 2 - 1)
                    conv_layer = nn.Conv1d(channel_size, channel_size, k, stride=1, padding=0)
                    # 为偶数核手动添加填充
                    padded_conv = nn.Sequential(
                        nn.ConstantPad1d(padding, 0),  # 添加对称填充
                        conv_layer
                    )
                    scales.append(padded_conv)
                else:
                    # 奇数卷积核直接计算填充
                    padding = k // 2
                    scales.append(nn.Conv1d(channel_size, channel_size, k, stride=1, padding=padding))
    ...
    def forward(self, x):
        ...
        for scales in self.heads:
            ...
            if any(am.shape[-1] != x.shape[-1] for am in attention_maps):
                raise RuntimeError(f"Mismatch in attention map size: expected {x.shape[-1]}")
...
# CNNTransformerModel 类定义
class CNNTransformerModel(nn.Module):
    def __init__(self, input_size, out_channels, kernel_sizes, num_heads, head_kernel_sizes, hidden_size, output_size, max_len, dropout_rate):
...
model = CNNTransformerModel(
    ...
    num_heads=hp.nhead16,  # 使用16个头
    head_kernel_sizes=hp.kernel_sizes_per_head16,  # 使用16组卷积核尺寸
    hidden_size=hp.hidden_size16,
