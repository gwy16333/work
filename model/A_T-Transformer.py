...
class TTransformerLayer(nn.Module):
...
class TTransformer(nn.Module):
    def __init__(self, input_size, d_model, num_heads, num_layers, output_size, max_len, dropout=0.1):
        ...
        # Transformer层堆叠
        self.layers = nn.ModuleList([TTransformerLayer(d_model, num_heads, dropout) for _ in range(num_layers)])
        # 输出层
        self.output_layer = nn.Linear(d_model, output_size)

    def forward(self, x):
        # 输入形状: [batch_size, seq_len, input_size]
        x = self.input_proj(x)  # 投影到隐藏维度 [batch, seq, d_model]
        ...
        # 输出预测 [batch, output_seq, output_size]
        output = self.output_layer(x)
        return output[:, -hp.output_max_len:, :]  # 截取最后预测的时间步
