...
def build_model(scarlr_, test_data):
    """
    Builds and fits a SARIMA model for each series in the test data.

    Args:
        scarlr_: Scaler used to inverse transform the data.
        test_data: Test data to model and forecast.

    Returns:
        predictions (np.array): Forecasts for the given test data.
    """
    test_data_ = scarlr_.inverse_transform(np.squeeze(test_data))
    predictions = []
    for i in range(test_data_.shape[0]):
        # Define a SARIMA model; adjust the seasonal_order to fit the dataset's seasonality
        model = SARIMAX(test_data_[i, :hp.maxlen],
                        order=(1, 0, 1),  # Non-seasonal order
                        seasonal_order=(2, 1, 0+
                                        9666663
                                        , 12))  # Seasonal order: (P, D, Q, S)
                                                    # Here, '12' could be replaced with the actual seasonality period of the dataset
        #model = SARIMAX(test_data_[i, :hp.maxlen],
        #                order=(1, 0, 1),  # Non-seasonal order
        #                seasonal_order=(1, 1, 1, 12))  # Seasonal order: (P, D, Q, S)


        sarima_res = model.fit(disp=False)  # Set disp=False to turn off debug information
        # Forecast the next 12 steps and append to predictions
        predictions.append(sarima_res.forecast(12))
