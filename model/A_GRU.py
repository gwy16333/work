...
class GRUModel(nn.Module):
    def __init__(self):
        ...
        self.gru = nn.GRU(input_size=hp.input_size, hidden_size=hp.hidden_size,
                          num_layers=hp.num_layers, batch_first=True)
        self.fc = nn.Linear(hp.hidden_size, hp.output_size)  # GRU输出层

    def forward(self, x):
        batch_size, seq_len, _ = x.size()
        x, _ = self.gru(x)  # x 的形状：[batch_size, seq_length, hidden_size]
...
model = GRUModel().to(device)  # 将模型移到 GPU
...
os.chdir('..')
# 将结果保存到Excel文件中
df.to_excel('./each_step_metrics_pems08/GRU_pytorch_60m.xlsx', index=False)
...
draw_gt_pred(sing_label, None)
draw_gt_pred(sing_label[288:288*2], None)
