...
class CNNTransformerModel(nn.Module):
    def __init__(self, hp):
        ...
        self.cnn_encoder = MultiScaleConvBlock(hp.input_size, hp.out_channels, hp.kernel_sizes)
        self.pos_encoder = PositionalEncoding(hp.hidden_size, hp.dropout_rate, hp.max_position_embeddings)
        encoder_layers = nn.TransformerEncoderLayer(d_model=hp.hidden_size, nhead=hp.nhead, dim_feedforward=hp.dim_feedforward,
                                                    dropout=hp.dropout_rate, activation=hp.activation_function, batch_first=True)
        self.transformer_encoder = nn.TransformerEncoder(encoder_layers, num_layers=hp.num_layers)
        # 因为MultiScaleConvBlock在通道维度上进行了拼接，所以输出通道数是原始输出通道数乘以卷积核数量
        self.fc1 = nn.Linear(hp.out_channels * len(hp.kernel_sizes), hp.hidden_size)
