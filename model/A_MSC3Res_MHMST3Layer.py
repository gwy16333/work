...
class MultiHeadMultiScaleAttention(nn.Module):
    def __init__(self, channel_size, head_kernel_sizes):
        ...
        for kernel_sizes in head_kernel_sizes:  # head_kernel_sizes 是一个列表的列表
...
class ResidualBlock(nn.Module):
    def __init__(self, in_channels, out_channels, kernel_sizes):
        ...
        self.multi_scale_conv = MultiScaleConvBlock(in_channels, out_channels, kernel_sizes)
        self.bn1 = nn.BatchNorm1d(out_channels * len(kernel_sizes))
        self.relu = nn.ReLU(inplace=True)
        self.conv2 = nn.Conv1d(out_channels * len(kernel_sizes), out_channels, 1)
...
class CNNEncoder(nn.Module):
    def __init__(self, hp):
        ...
        current_in_channels = hp.input_size
        for kernel_sizes in hp.kernel_sizes_per_head:
            self.layers.append(ResidualBlock(current_in_channels, hp.out_channels, kernel_sizes))
            # 更新下一个ResidualBlock的输入通道数
            current_in_channels = hp.out_channels
...
class CNNTransformerModel(nn.Module):
    def __init__(self, hp):
        ...
        self.cnn_encoder = nn.Sequential(
            ResidualBlock(hp.input_size, hp.out_channels, hp.kernel_sizes_per_head[0]),
            ResidualBlock(hp.out_channels, hp.out_channels, hp.kernel_sizes_per_head[1]),  # 第二个残差块
            ResidualBlock(hp.out_channels, hp.out_channels, hp.kernel_sizes_per_head[2]),
            # 可以根据需要继续添加更多残差块
        )

        # 使用 hp.kernel_sizes_per_head 为每个头指定不同的卷积核尺寸
        self.multi_scale_attention = MultiHeadMultiScaleAttention(hp.out_channels, hp.kernel_sizes_per_head)

        self.pos_encoder = PositionalEncoding(hp.hidden_size, hp.dropout_rate, hp.max_position_embeddings)

        # 创建两个Transformer编码器层，以增加残差连接
        self.transformer_encoder = nn.TransformerEncoder(
            nn.TransformerEncoderLayer(d_model=hp.hidden_size, nhead=hp.nhead,
                                       dim_feedforward=hp.dim_feedforward, dropout=hp.dropout_rate, batch_first=True),
            num_layers=3  # 增加编码器层数量以增加残差连接
        )

        self.fc1 = nn.Linear(hp.out_channels * hp.nhead, hp.hidden_size)
        self.fc2 = nn.Linear(hp.hidden_size, hp.output_size)

    def forward(self, x):
        x = x.transpose(1, 2)  # 调整维度
        x = self.cnn_encoder(x)
        x = self.multi_scale_attention(x)  # 应用多尺度注意力
...
os.chdir('..')
# 将结果保存到Excel文件中
df.to_excel('./each_step_metrics_pems08/CNN+Transformer-coding1_pytorch_60m.xlsx', index=False)
