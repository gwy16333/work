...
class TimeSformerLayer(nn.Module):
    ...
    def forward(self, src):
        ...
        # 时间注意力修正部分（移除维度转置）
        src2 = self.norm2(src)
        attn_output, _ = self.temporal_attn(src2, src2, src2)  # 保持维度 [batch, time, features]
...
class TimeSformerModel(nn.Module):
    def __init__(self, input_size, d_model, num_heads, num_layers, output_size, max_len, dropout=0.1):
        ...
        self.layers = nn.ModuleList([TimeSformerLayer(d_model, num_heads, dropout) for _ in range(num_layers)])
        self.output_layer = nn.Linear(d_model, output_size)

    def forward(self, x):
        # x形状: [batch, seq_len, input_size]
        x = self.input_proj(x)  # 投影到隐藏维度
        ...
        # 输出预测
        output = self.output_layer(x)  # [batch, seq_len, output_size]
        # 取最后output_max_len个时间步
        output = output[:, -hp.output_max_len:, :]
        return output
