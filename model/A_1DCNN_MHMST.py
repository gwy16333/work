...
class CNNEncoder(nn.Module):
    def __init__(self, hp):
        ...
        for i in range(hp.num_cnn_layers):
            in_channels = hp.input_size if i == 0 else hp.out_channels
            out_channels = hp.out_channels
            # 调整最后一个卷积层的输出通道数
            if i == hp.num_cnn_layers - 1:
                out_channels = 256  # 根据MultiHeadMultiScaleAttention的输入需求调整
            self.conv_layers.append(
                nn.Conv1d(in_channels=in_channels,
                          out_channels=out_channels,
                          kernel_size=hp.kernel_size,
                          padding=hp.kernel_size//2))
...
class CNNTransformerModel(nn.Module):
    def __init__(self, input_size, out_channels, kernel_sizes, head_kernel_sizes, num_heads, hidden_size, output_size, max_len, dropout_rate):
        ...
        self.cnn_encoder = CNNEncoder(hp)  # 使用适当定义的CNNEncoder
